...
public class ObjectFactoryImpl implements es.carm.openldap.dao.ObjectFactory, Serializable {
	...
	private boolean configurate(Properties prop) {
		...
		if ( ( "yes".equalsIgnoreCase( prop.getProperty("config.ldap.connection.use_ssl") )) || ( "si".equalsIgnoreCase( prop.getProperty("config.ldap.connection.use_ssl") )) ) {
			useSSL=true;
		}
		
		return (null!=config_Ldap_URL) && (null!=config_Bind_DN) && (null!=config_Bind_Password) && (null!=config_BaseDN);
	}
	...
	public FactoryDao<UsuarioDAC> getUsuariosDAO() {
		return new UserFactoryDAOImpl( new PooledConnectionFactory(pool) , config_BaseDN, config_FilterSearchUsers );
	}
