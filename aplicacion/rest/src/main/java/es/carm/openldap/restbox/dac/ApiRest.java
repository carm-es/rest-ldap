...
public class ApiRest {
	...
	@Produces({"text/json","application/json","text/xml","application/xml","application/directory","text/ldif","text/dsml"})
	public Response getUsers(@HeaderParam("Accept") String header, @PathParam("userid") String id, @PathParam("formato") String ext, @QueryParam("offset") int offsetParam, @QueryParam("limit") int limitParam, @QueryParam("order") String orderParam) {
		...
		if ( null == instFactory ) {
			return Response.serverError().type(mimeResult).entity( Settings.getErrorResponse("No se configuró LDAP", mimeResult) ).header( "Content-Type", contentType) .header("Access-Control-Allow-Origin", "*").build();
		}
		
		// Obtener la factoria de grupos...
		FactoryDao<UsuarioDAC> factory=instFactory.getUsuariosDAO();
		if ( null == factory ) {
			return Response.serverError().type(mimeResult).entity( Settings.getErrorResponse("No se pudo obtener FactoryDao<UsuarioDAC> de la configuración", mimeResult) ).header( "Content-Type", contentType).header("Access-Control-Allow-Origin", "*").build();
		}
		...
		if ( ( null != id ) && ( 0< id.trim().length() ) ) {
			...
			try {				
				...
				return Response.serverError().type(mimeResult).entity( Settings.getErrorResponse( "Error al buscar '"+ id +"' en el directorio", mimeResult) ).header( "Content-Type", contentType) .header("Access-Control-Allow-Origin", "*").build();
				
			} catch (Exception x) {
				log.error("Error indeterminado, quizás se deba a que no está configurado el POOL", x );
				return Response.serverError().type(mimeResult).entity( Settings.getErrorResponse( "La aplicación no está bien configurada", mimeResult) ).header( "Content-Type", contentType) .header("Access-Control-Allow-Origin", "*").build();
			}
		...
		if ( (null!=lista) && ( 0< lista.size()) ) {			
			return Response.ok(getResult(lista.get(0), mimeResult ) , mimeResult ).header( "Content-Type", contentType) .header("Access-Control-Allow-Origin", "*").build();
			
		}
		
		// Salida sin resultado...
		return Response.status(Response.Status.NOT_FOUND).type(mimeResult).entity( Settings.getErrorResponse("No se encontró '"+ id +"' en el directorio", mimeResult) ).header( "Content-Type", contentType) .header("Access-Control-Allow-Origin", "*").build();
