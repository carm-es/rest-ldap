...
public class ApiRest {
  ...
  @Produces({"text/json", "application/json", "text/xml", "application/xml",
      "application/directory", "text/ldif", "text/dsml"})
  public Response getUsers(@HeaderParam("Accept") String header, @PathParam("userid") String id,
    ...
    // Preparar la respuesta, en base a lo que acepta el cliente, y la extensión que nos han
    // pedido...
    ...
    if (null == factory) {
      return Response.serverError().type(mimeResult)
          .entity(Settings.getErrorResponse(
              "No se pudo obtener FactoryDao<UsuarioDAC> de la configuración", mimeResult))
          .header("Content-Type", contentType).header("Access-Control-Allow-Origin", "*").build();
    }
    ...
    if ((null != id) && (0 < id.trim().length())) {
      ...
      try {
        ...
        return Response.serverError().type(mimeResult)
            .entity(Settings.getErrorResponse("Error al buscar '" + id + "' en el directorio",
                mimeResult))
            .header("Content-Type", contentType).header("Access-Control-Allow-Origin", "*").build();
    ...
    if ((null != lista) && (0 < lista.size())) {
      return Response.ok(getResult(lista.get(0), mimeResult), mimeResult)
          .header("Content-Type", contentType).header("Access-Control-Allow-Origin", "*").build();
    ...
    return Response.status(Response.Status.NOT_FOUND).type(mimeResult)
        .entity(
            Settings.getErrorResponse("No se encontró '" + id + "' en el directorio", mimeResult))
        .header("Content-Type", contentType).header("Access-Control-Allow-Origin", "*").build();
